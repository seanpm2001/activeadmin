...
ActiveAdmin.register Post do
  ...
  batch_action :set_starred, partial: "starred_batch_action_form", link_html_options: { "data-modal-toggle": "starred-batch-action-modal" } do |ids, inputs|
    ...
    redirect_to collection_path(user_id: params[:user_id]), notice: [ids, inputs].to_s
  end

  batch_action :second_partial, partial: "starred2_batch_action_form", link_html_options: { "data-modal-toggle": "starred2-batch-action-modal" } do |ids, inputs|
    pp inputs
    # Post.where(id: ids).update_all(starred: inputs["starred"].present?)
    # redirect_to collection_path(user_id: params["user_id"]), notice: "The posts have been updated."
    redirect_to collection_path(user_id: params[:user_id]), notice: [ids, inputs].to_s
    ...
    tabs class: "mt-6" do
      tab :profile do
        para class: "text-sm text-gray-500 dark:text-gray-400" do
          text_node "This is some placeholder content the "
          strong "Profile tab's associated content.", class: "font-medium text-gray-800 dark:text-white"
          text_node "Clicking another tab will toggle the visibility of this one for the next. The tab JavaScript swaps classes to control the content visibility and styling."
        end
        # footer do
        #   para "test"
        # end
      ...
      tab :dashboard do
        para class: "text-sm text-gray-500 dark:text-gray-400" do
          text_node "This is some placeholder content the "
          strong "Dashboard tab's associated content.", class: "font-medium text-gray-800 dark:text-white"
          text_node "Clicking another tab will toggle the visibility of this one for the next. The tab JavaScript swaps classes to control the content visibility and styling."
        end
      end
      tab :settings do
        para "The dashboard tab."
      end
      tab :contacts do
        para "The dashboard tab."
      end
  ...
  form do |f|
    ...
    f.inputs "Tags" do
      ...
      # f.has_many :taggings, remove_record: "Hide" do |t|
